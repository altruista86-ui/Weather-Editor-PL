...
static void weather_editor_saved_profiles_build_list(WeatherStationApp* app) {
    ...
    if(!storage) {
        submenu_set_header(app->submenu, "Blad karty SD");
        view_dispatcher_switch_to_view(app->view_dispatcher, WeatherStationViewSubmenu);
        weather_station_release_inactive_gui_views(app, WeatherStationViewSubmenu);
        return;
    }
    ...
    if(total_count == 0U) {
        ...
        snprintf(empty_header, sizeof(empty_header), "%s - brak zapisow", source_label);
...
bool weather_station_scene_saved_profiles_on_event(void* context, SceneManagerEvent event) {
    ...
    if(!weather_station_ensure_widget(app)) {
        furi_string_set(app->editor_result_title, "Brak pamieci");
        furi_string_set(app->editor_result_text, "Nie mozna otworzyc widoku ladowania.");
        return true;
    }
    widget_reset(app->widget);
    widget_add_string_element(
        app->widget, 64, 29, AlignCenter, AlignCenter, FontPrimary, "Wczytywanie...");
    ...
    if(ok) {
        ...
        furi_string_set(app->editor_status, "Brak pamieci na nazwe presetu");
    }
    ...
    if(ok) {
        ...
        furi_string_set(app->editor_result_title, "Blad odczytu");
