...
void weather_station_scene_simulation_on_enter(void* context) {
    ...
    submenu_set_header(app->submenu, "Symulacja - protokol");
...
bool weather_station_scene_simulation_on_event(void* context, SceneManagerEvent event) {
    ...
    if(!weather_station_ensure_radio_core(app)) {
        furi_string_set(app->editor_result_title, "Brak radia");
        furi_string_set(app->editor_result_text, "Nie mozna uruchomic Sub-GHz.");
    ...
    if(!weather_editor_simulation_prepare(
        ...
        furi_string_set(app->editor_result_title, "Blad symulacji");
        furi_string_set(app->editor_result_text, "Nie mozna przygotowac protokolu.");
        scene_manager_next_scene(app->scene_manager, WeatherStationSceneActionResult);
        return true;
    }
