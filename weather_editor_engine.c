...
bool weather_editor_encode(const WeatherEditorState* state, WeatherEditorRaw* out_raw, FuriString* status) {
    ...
    if(!state || !state->protocol_name || !out_raw || !out_raw->values ||
       out_raw->capacity == 0U) {
        if(status) furi_string_set(status, "Brak danych do kodowania");
        return false;
    }
    ...
    if(protocol_is(state, "Nexus-TH")) encode_nexus(state, out_raw, &data);
        ...
        if(!weather_editor_encode_extended(state, out_raw, &data, &handled, status)) {
            if(!handled && status)
                furi_string_set(status, "Dekoder RX jest\nBrak edycji TX");
            return false;
        }
    }

    if(out_raw->overflowed) {
        if(status) furi_string_set(status, "Burst TX za dlugi");
        return false;
    }
    ...
    if(status) {
        if(state->overflow_wrapped)
            furi_string_set(status, "Zakodowano z przepelnieniem");
        else
            furi_string_set(status, "Ramka edytowana gotowa");
    }
...
bool weather_editor_save_raw_sub(
    ...
    if(!path || !path[0] || !preset || !preset->name || !raw || !raw->values ||
       raw->count == 0U) {
        if(status) furi_string_set(status, "Brak danych RAW do zapisu");
        return false;
    }

    Storage* storage = furi_record_open(RECORD_STORAGE);
    if(!storage) {
        if(status) furi_string_set(status, "Brak pamieci Storage");
        return false;
    }
    ...
    if(!fff) {
        ...
        if(status) furi_string_set(status, "Brak pamieci pliku SUB");
        return false;
    }
    ...
    do {
        ...
        if(!strcmp(preset_name, "FuriHalSubGhzPresetCustom")) {
            if(!preset->data || preset->data_size == 0U) {
                if(status) furi_string_set(status, "Brak danych presetu CUSTOM");
                break;
            }
    ...
    if(status) {
        if(ok) furi_string_set(status, "Zapisano plik RAW .sub");
        else if(furi_string_size(status) == 0U) furi_string_set(status, "Blad zapisu .sub");
    }
    return ok;
}
...
bool weather_editor_save_key_sub(
    ...
    if(!path || !path[0] || !preset || !preset->name || !state || !state->protocol_name) {
        if(status) furi_string_set(status, "Brak danych .ws");
        return false;
    }
    ...
    if(!stream || !text) {
        ...
        furi_record_close(RECORD_STORAGE);
        if(status) furi_string_set(status, "Brak pamieci .ws");
        return false;
    }
    ...
    if(!preset_name) {
        ...
        furi_record_close(RECORD_STORAGE);
        if(status) furi_string_set(status, "Brak presetu .ws");
        return false;
    }
    ...
    furi_record_close(RECORD_STORAGE);
    if(status) {
        if(ok) furi_string_set(status, "Zapisano Weather .ws");
        else furi_string_set(status, "Blad zapisu Weather .ws");
    }
    return ok;
}
...
bool weather_editor_save_profile(
    ...
    if(status) furi_string_reset(status);
    if(!path || !path[0] || !preset || !preset->name || !state || !state->protocol_name) {
        if(status) furi_string_set(status, "Brak danych profilu");
        return false;
    }
    ...
    if(!fff) {
        furi_record_close(RECORD_STORAGE);
        if(status) furi_string_set(status, "Brak pamieci profilu");
        return false;
    }
    bool ok = false;
    do {
    ...
    if(status) {
        if(ok) furi_string_set(status, "Zapisano profil danych");
        else if(furi_string_size(status) == 0) furi_string_set(status, "Blad zapisu profilu");
    }
    return ok;
}
...
bool weather_editor_load_profile(
    ...
    *custom_preset_data = NULL;
    ...
    do {
        ...
        if(!opened) {
            if(status) furi_string_set(status, "Nie mozna otworzyc profilu");
            break;
        }
        if(!flipper_format_read_header(fff, filetype, &version) || version != 1U) {
            if(status) furi_string_set(status, "Nieprawidlowy zapis Weather");
            break;
        }
        ...
        if(strcmp(furi_string_get_cstr(filetype), "Flipper Weather Station Key File") == 0) {
            ...
            if(flipper_format_get_value_count(fff, "Custom_preset_data", &preset_count) && preset_count) {
                if(preset_count > WEATHER_EDITOR_MAX_PRESET_DATA) {
                    if(status) furi_string_set(status, "Preset .ws jest za duzy");
                    break;
                }
                new_preset_data = malloc(preset_count);
                if(!new_preset_data) {
                    if(status) furi_string_set(status, "Brak pamieci na preset");
                    break;
                }
                flipper_format_rewind(fff);
        ...
        if(strcmp(furi_string_get_cstr(filetype), "Weather Lab Profile") != 0) {
            if(status) furi_string_set(status, "Nieobslugiwany zapis Weather");
            break;
        }
        ...
        if(flipper_format_get_value_count(fff, "CustomPresetData", &preset_count) && preset_count) {
            if(preset_count > WEATHER_EDITOR_MAX_PRESET_DATA) {
                if(status) furi_string_set(status, "Preset profilu jest za duzy");
                break;
            }
            new_preset_data = malloc(preset_count);
            if(!new_preset_data) {
    ...
    if(status) {
        if(ok) furi_string_set(status, "Profil wczytany");
        else if(furi_string_size(status) == 0) furi_string_set(status, "Blad odczytu profilu");
    }
    return ok;
}
