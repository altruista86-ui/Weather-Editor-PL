...
bool weather_editor_validate_raw(
    ...
    if(!app || !app->txrx || !app->txrx->receiver || !raw || !raw->values ||
       !expected_state || !expected_state->protocol_name) {
        if(status) furi_string_set(status, "Test TX pominiety: brak rdzenia RX");
        return false;
    }
    ...
    if(!loopback.decoded) {
        if(status) furi_string_set(status, "Test TX: brak rozpoznanej ramki");
        return false;
    }
    ...
    if(status) {
        furi_string_printf(
            status, "Test TX OK (%u ramek)\nBity i pola zgodne", loopback.decoded_frames);
    }
    return true;
}
...
bool weather_editor_send_raw(
    ...
    if(!app || !app->txrx || !raw || !raw->values || !preset || !preset->name) {
        if(status) furi_string_set(status, "Brak danych TX");
        return false;
    }

    if(!app->txrx->radio_device) {
        if(status) furi_string_set(status, "Brak radia Sub-GHz");
        return false;
    }

    if(raw->count == 0) {
        if(status) furi_string_set(status, "Brak danych RAW");
        return false;
    }

    if(!subghz_devices_is_frequency_valid(app->txrx->radio_device, preset->frequency)) {
        if(status) furi_string_set(status, "Niedozwolona czestotliwosc");
        return false;
    }

    uint8_t* preset_data = weather_editor_get_preset_data(app, preset);
    if(!preset_data) {
        if(status) furi_string_set(status, "Brak danych presetu");
        return false;
    }
    ...
    do {
        ...
        if(!subghz_devices_set_tx(app->txrx->radio_device)) {
            if(status) furi_string_set(status, "TX zablokowany dla pasma");
            break;
        }
        ...
        if(!subghz_devices_start_async_tx(
            ...
            if(status) furi_string_set(status, "Nie mozna uruchomic TX");
            break;
        }
        ...
        if(status) {
            furi_string_set(
                status, ok ? "Transmisja zakonczona" : "Timeout transmisji");
        }
    } while(false);
    ...
    if(status && !ok && furi_string_size(status) == 0) {
        furi_string_set(status, "Blad TX");
    }
    return ok;
}
