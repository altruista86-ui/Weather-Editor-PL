...
void ws_view_receiver_info_draw(Canvas* canvas, WSReceiverInfoModel* model) {
    ...
    if(model->generic->channel != WS_NO_CHANNEL) {
        snprintf(buffer, sizeof(buffer), "Kan: %01d", model->generic->channel);
    ...
    if(model->generic->btn != WS_NO_BTN) {
        snprintf(buffer, sizeof(buffer), "Prz: %01d", model->generic->btn);
    ...
    snprintf(buffer, sizeof(buffer), "Dane: 0x%llX", model->generic->data);
    ...
    if((int)model->generic->timestamp > 0 && model->curr_ts) {
        ...
        if(ts_diff > 60) {
            ...
            if(model->curr_ts % 2 == 0) {
                canvas_draw_str_aligned(canvas, 105, 51, AlignLeft, AlignCenter, "Stary");
            } else {
                if(cnt_min >= 59) {
                    canvas_draw_str_aligned(canvas, 105, 51, AlignLeft, AlignCenter, "Stary");
                } else {
